...
import net.hollowbit.archipeloserver.world.Tile;
...
public class FootstepPlayerComponent extends EntityComponent {
	...
	public void tick20(float deltaTime) {
		...
		Tile tile = ArchipeloServer.getServer().getMapElementManager().getTile((entity.getMap().getTile((int) tilePos.x, (int) tilePos.y)));
		if (tile != null) {
			String tileSound = tile.getFootstepSound();
